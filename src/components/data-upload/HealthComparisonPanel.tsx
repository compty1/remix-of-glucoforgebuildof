...
import { 
  Heart, Activity, Brain, Eye, Shield, 
...
const HealthComparisonPanel: React.FC<HealthComparisonPanelProps> = ({ detailedAnalysis }) => {
  ...
  // Memoize all static arrays so they are not re-created on every render (Issue 134)
  // Use ADA T1D-appropriate targets (NOT non-diabetic benchmarks) (Issue 44, 203)
  ...
  ], [userTIR, userAvg, userCV, userGMI, userTimeHigh, userTimeLow]);

  const healthImpacts = useMemo(() => [
  ...
  ], [userGMI, userCV, userTimeLow]);

  const mentalHealthImpacts = useMemo(() => [
  ...
  ], [userGMI, userCV, userTimeHigh]);

  const lifestylePlans = useMemo(() => [
